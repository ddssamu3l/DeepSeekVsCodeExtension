...
class LoCopilotViewProvider {
    ...
    _workspaceUpdateTimer;
    /** Tracks context window usage */
    _contextInfo = { used: 0, total: 8192 };
...
When the user asks a question, focus on providing direct, practical answers that address their specific needs.`,
     ...
     * @private
     */
    async _initWorkspaceMap() {
        try {
            // Build prioritized workspace map with a reasonable file size limit
            const workspaceMap = await (0, workspaceUtils_1.buildPrioritizedWorkspaceMap)(100); // 100KB max per file
            if (workspaceMap.length > 0) {
                ...
                const mapString = (0, workspaceUtils_1.createCondensedMapForPrompt)(workspaceMap);
    ...
    async refreshWorkspaceMap() {
        try {
            ...
            const workspaceMap = await (0, workspaceUtils_1.buildPrioritizedWorkspaceMap)(50);
            if (workspaceMap.length > 0) {
                // Create a condensed version suitable for the system prompt
                const mapString = (0, workspaceUtils_1.createCondensedMapForPrompt)(workspaceMap);
     ...
     * @private
     */
    async _checkOllamaInstalled() {
        if (!this._view) {
            console.error("Cannot check Ollama - view is undefined");
            return false;
        }
        return await (0, platformUtils_1.checkOllamaInstalled)(this._view);
    }
    /**
    ...
    async _isWSL() {
        ...
        return new Promise((resolve) => {
            ...
            exec('grep -i microsoft /proc/version', (error1, stdout1) => {
                ...
                exec('uname -r', (error2, stdout2) => {
                    ...
                    exec('ls -la /mnt/c 2>/dev/null', (error3) => {
                        if (!error3) {
            ...
            if (isWSL) {
                ...
                exec('where ollama', (error, stdout) => {
                    if (error || !stdout.trim()) {
                        ...
                        resolve(commonPaths[0]); // Use first common path as fallback
                    }
                    else {
                        resolve(stdout.trim());
                    }
                ...
                // On macOS/Linux, just use the command name as it should be in PATH
                resolve('ollama');
            }
        });
    }
    ...
    _buildFullPrompt(userPrompt) {
        const { selectedText } = (0, editorUtils_1.default)();
     ...
     * @private
     */
    async _getModelContextSize() {
        try {
            // First check if the model is installed
            try {
                await ollama_1.default.show({ model: this._currentModel });
            }
            catch (error) {
            ...
            const modelInfo = await ollama_1.default.show({ model: this._currentModel });
     ...
     */
    _updateContextUsage(promptTokens) {
        ...
        try {
            ...
            for await (const part of streamResponse) {
                ...
                if (firstChunk) {
                    ...
                    const promptEvalCount = part.prompt_eval_count;
     ...
     * @param {string} userPrompt - The user's prompt text
     */
    _trackReferencedFiles(userPrompt) {
        try {
            ...
            for (const fileName of potentialFiles) {
                ...
                vscode.workspace.findFiles(`**/${fileName}`).then(uris => {
                    for (const uri of uris) {
                        (0, workspaceUtils_1.recordFileAccess)(uri.fsPath);
                    }
                });
            }
            // Also track the current file
            if (vscode.window.activeTextEditor?.document?.uri?.fsPath) {
                (0, workspaceUtils_1.recordFileAccess)(vscode.window.activeTextEditor.document.uri.fsPath);
            }
        }
        catch (error) {
     ...
     * @private
     */
    async _installOllamaWithTerminal() {
        try {
            const terminal = vscode.window.createTerminal("Ollama Installation");
            // Check for WSL and platform
            const wslDetected = await (0, platformUtils_1.isWSL)() || (0, platformUtils_1.detectWSLFromEnvironment)();
            const platform = process.platform;
            console.log("Installation environment - Platform:", platform, "WSL:", wslDetected);
            // Add debug info to terminal
            (0, platformUtils_1.addTerminalDebugInfo)(terminal, wslDetected);
            // Use the appropriate installation method based on environment
            if (wslDetected) {
                (0, platformUtils_1.installOllamaInWSL)(terminal);
            }
            else if (platform === 'darwin') {
                (0, platformUtils_1.installOllamaOnMacOS)(terminal);
            }
            else if (platform === 'linux') {
                (0, platformUtils_1.installOllamaOnLinux)(terminal);
            }
            else if (platform === 'win32') {
                (0, platformUtils_1.installOllamaOnWindows)(terminal);
            }
            else {
